...
def signature(document, generator_matrix_systematic, parity_check, inverse_parity_check):
    ...
    syndrome=document.dot(parity_check.transpose())
...
#this part finds a right inverse of the parity check matrix by first multiplying
#the parity check matrix by its transpose. It then finds the inverse of the result.
#The obtained matrix is then multiplied from the left by the transpose of the parity check matrix
#and the result is a right inverse of the parity check matrix.

H_times_H_trans=official_parity_check.dot(official_parity_check.transpose())%2
              
X=inverse(H_times_H_trans)
parity_check_inverse=(official_parity_check.transpose()).dot(X)%2
round_2_time=time.process_time()-start_time_2
#check=official_parity_check.dot(parity_check_inverse)%2
...
for i in range(rounds):
    ...
    sign, syndrome = signature(document, generator_matrix, official_parity_check, parity_check_inverse)
    ver=verify(sign, syndrome, document, generator_matrix, parity_check_inverse)
    if (ver == False):
        break
